<script lang="ts">
  ...
  $: if (leaderboard) updateLeaderboardTexture();

  function createLeaderboardTexture() {
    if (typeof document === 'undefined') return new CanvasTexture(undefined as any); // SSR guard
  ...
  function generateDroneTargets(p: string) {
  ...
  export function startFlyingTour() {
    isGameMode.value = true;
  ...
  function animate() {
    ...
    const isDroneModeInst = gameModeManager.getMode() instanceof DroneMode;

    if (drones && !isDroneModeInst) {
      const positions = drones.geometry.attributes.position.array;
  ...
  onMount(() => {
    ...
    trafficSystem = new TrafficSystem(scene, 150, occupiedGrids, spawnSparks);
...
<div bind:this={canvasContainer} id="cyberpunk-city"></div>
<GameUI
