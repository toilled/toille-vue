...
<ul>
  ...
  <li class="icons-container">
    ...
    <div on:click={() => dispatch('explore')} class="icon-wrapper" title="Explore City">
      <img src="/person-icon.svg" alt="Explore City" class="icon" />
    </div>
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div on:click={() => dispatch('fly')} class="icon-wrapper" title="Fly Tour">
      <img src="/plane-icon.svg" alt="Fly Tour" class="icon" />
    </div>
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div on:click={toggleSound} class="icon-wrapper" title="Toggle Sound">
      {#if soundOn}
        <img
          src="/sound-icon.svg"
          alt="Toggle sound"
    ...
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div on:click={() => dispatch('toggle-content')} class="icon-wrapper" title="Toggle Visibility">
       {#if contentVisible}
...
ul {
  position: relative;
  /* Reset default UL styling if needed, but Pico handles some */
  list-style: none;
  padding: 0;
  margin: 0;
}
