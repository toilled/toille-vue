...
export default function Menu(props: MenuProps) {
  const soundOn = createMemo(() => audioManager.isSoundEnabled());
  ...
  return (
    <ul>
      ...
      <li class="icons-container">
        <div onClick={props.onExplore} class="icon-wrapper" title="Explore City">
          <img src="/person-icon.svg" alt="Explore City" class="icon" />
        </div>
        <div onClick={props.onFly} class="icon-wrapper" title="Fly Tour">
          <img src="/plane-icon.svg" alt="Fly Tour" class="icon" />
        </div>
        <div onClick={props.onDemo} class="icon-wrapper" title="64k Demo">
          <img src="/64k-icon.svg" alt="64k Demo" class="icon" />
        </div>
        <div onClick={toggleSound} class="icon-wrapper" title="Toggle Sound">
          <Show
            when={soundOn()}
            fallback={<img src="/mute-icon.svg" alt="Toggle sound" class="icon" />}
          >
            <img src="/sound-icon.svg" alt="Toggle sound" class="icon" />
          </Show>
        </div>
        <div onClick={props.onToggleContent} class="icon-wrapper" title="Toggle Visibility">
          <Show
            when={props.contentVisible}
            fallback={
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#000000" class="icon">
