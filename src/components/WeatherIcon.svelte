<script lang="ts">
  ...
  let graphPoints = '';
  $: graphPoints = computedPoints.map(p => `${p.x},${p.y}`).join(' ');
...
<div class="icon-wrapper" title={description} on:click={() => { if (hourlyForecast.length) showModal = true; }}>
    {#if iconType === 'sun'}
    ...
    {#if showModal}
      <div use:portal class="weather-modal-overlay" on:click|self={() => showModal = false}>
        <article class="weather-modal">
          <header class="modal-header">
            <h3>Weather Forecast</h3>
            <button class="close-btn" on:click={() => showModal = false} aria-label="Close">&times;</button>
          </header>
          <div class="chart-container">
            <svg viewBox="0 0 300 150" class="weather-chart">
              <line x1="0" y1="135" x2="300" y2="135" stroke="#444" stroke-width="1" />

              {#each computedPoints as point, i}
                 <g>
                     <rect
                        x={point.x - 5}
                        y={point.rainY}
