...
describe('Checker.vue', () => {
  ...
  it('initializes count to 0', () => {
    const wrapper = mount(Checker);
    expect((wrapper.vm as unknown as Record<string, unknown>).count).toBe(0);
  });
  ...
  it('increments count when add is clicked', async () => {
    ...
    expect((wrapper.vm as unknown as Record<string, unknown>).count).toBe(1);
  });
  ...
  it('updates times correctly when count changes', async () => {
    ...
    // Initial state
    expect((wrapper.vm as unknown as Record<string, unknown>).limitTime).toBe(
      new Date().toLocaleTimeString([], options)
    );
    expect((wrapper.vm as unknown as Record<string, unknown>).soberTime).toBe(
      new Date().toLocaleTimeString([], options)
    );
    ...
    expect((wrapper.vm as unknown as Record<string, unknown>).soberTime).toBe(
      new Date(currentTime + 2 * 60 * 60 * 1000).toLocaleTimeString([], options)
    );
  });
});
