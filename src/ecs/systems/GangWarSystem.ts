...
import { GangWarCombat } from '../../game/GangWarCombat';
import { GangWarMarkers } from '../../game/GangWarMarkers';
import { Warrior, Projectile, type GangConfig, GANGS } from '../../game/GangWarTypes';
import { GRID_SIZE, BLOCK_SIZE, CELL_SIZE, START_OFFSET } from '../../game/config';

export class GangWarSystem extends System {
  private scene: Scene;
  private warriors: Warrior[] = [];
  private projectiles: Projectile[] = [];
  private fightMarkers: Mesh[] = [];
  private _spawnSparks: (pos: Vector3) => void;
  private _playPewSound: (pos?: Vector3) => void;
  private updateCounter = 0;
  private warriorsSpawned = false;
  ...
  constructor(
    scene: Scene,
    occupiedGrids: Map<string, { halfW: number; halfD: number; isRound?: boolean }>,
    _spawnSparks: (pos: Vector3) => void,
    _playPewSound: (pos?: Vector3) => void
  ) {
    super();
    this.scene = scene;
    this.occupiedGrids = occupiedGrids;
    this._spawnSparks = _spawnSparks;
    this._playPewSound = _playPewSound;
  ...
  init(_world: ECSWorld) {
    // Resources are accessed directly from the class
  }

  update(_world: ECSWorld, dt: number, _time: number): void {
    this.ensureWarriorsSpawned();
    this.updateCounter++;
  ...
  ensureWarriorsSpawned() {
    if (this.warriorsSpawned) return;
    this.warriorsSpawned = true;
    this.initWarriors();
  }
