...
export class MultiplayerSystem extends System {
  ...
  private onlineCountRef: { value: number };
  private lastBroadcastTime = 0;
  private broadcastInterval = 100;

  constructor(scene: Scene, onlineCountRef: { value: number }, carFactory: CarFactory) {
    super();
    this.scene = scene;
  ...
  private updateOnlineCount() {
    ...
    if (this.worldRef?.resources.multiplayer) {
      this.worldRef.resources.multiplayer.connected = !!this.client?.connected;
    }
  }

  private worldRef: ECSWorld | null = null;
  ...
  update(world: ECSWorld, _dt: number, _time: number): void {
    ...
    if (world.resources.multiplayer) {
      world.resources.multiplayer.players = this.players;
    }
  }
