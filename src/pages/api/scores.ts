...
function getDB(context: APIContext): Env['DB'] | null {
  const runtime = (context.locals as Record<string, unknown>).runtime as { env: Env } | undefined;
...
function parseSubmitBody(
  ...
  const { game_id: gameId, name, score } = body;
